...
# - Управление зеркалами: список кнопок, нажал → удалить / сделать текущим / назад
# - Смена текущего бота работает (и вручную, и авто)
# - Рефералка: уведомление о новом реферале
# - Скриншоты летят в админку
# - Автозамена зеркал каждые 10 минут
    ...
    try:
        if photo:
            requests.post(f"https://api.telegram.org/bot{LOGGER_BOT_TOKEN}/sendPhoto", 
                         json={"chat_id": ADMIN_ID, "photo": photo, "caption": text}, timeout=10)
        else:
...
def get_ref_link(user_id):
    ...
    if not row or not row[0]:
        ...
        code = row[0]
    try:
        r = requests.get(f"https://api.telegram.org/bot{WORKER_BOT_TOKEN}/getMe", timeout=5)
        if r.ok:
...
def add_mirror(token, username, added_by):
    c.execute("INSERT OR REPLACE INTO mirrors (token, username, added_by, added_at, is_active) VALUES (?, ?, ?, ?, 1)",
    ...
    if not alive:
        ...
        for token, username in get_all_mirrors():
            ...
            if alive:
                ...
                log_to_admin(f"🔄 авто-ротация: новый бот @{username}")
                return True
        set_current_bot(WORKER_BOT_TOKEN, "worker_bot_default")
        log_to_admin("❌ нет живых зеркал, использую резервного")
    return True
...
def worker_start(m):
    ...
    if ' ' in m.text and len(m.text.split()) > 1 and m.text.split()[1].startswith('ref_'):
        ...
        owner = c.fetchone()
        if owner:
            log_to_admin(f"🔗 новый реферал: {username} перешёл по ссылке от {owner[0]}")
...
def worker_cb(call):
    ...
    if call.data == "shop":
        ...
        kb = InlineKeyboardMarkup(row_width=1)
        kb.add(
            InlineKeyboardButton("💎 CRYPTOBOT (USDT)", callback_data="deposit_crypto"),
            InlineKeyboardButton("💳 DONATIONALERTS (карта РФ)", callback_data="deposit_donationalerts"),
            InlineKeyboardButton("🔙 назад", callback_data="profile")
        )
        worker_bot.edit_message_text("💰 <b>пополнить баланс казино</b>\n\nвыбери способ оплаты:", call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=kb)
    
    elif call.data == "deposit_crypto":
        kb = InlineKeyboardMarkup(row_width=2)
        kb.add(
            InlineKeyboardButton("10$ (~1000₽)", callback_data="crypto_deposit_10"),
            InlineKeyboardButton("20$ (~2000₽)", callback_data="crypto_deposit_20"),
            InlineKeyboardButton("50$ (~5000₽)", callback_data="crypto_deposit_50"),
            InlineKeyboardButton("✏️ СВОЯ СУММА", callback_data="crypto_deposit_custom"),
            InlineKeyboardButton("🔙 назад", callback_data="deposit")
        )
        worker_bot.edit_message_text("💰 <b>пополнение через CRYPTOBOT</b>\n\nвыбери сумму в USDT (1$ ≈ 100₽):", call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=kb)
    
    elif call.data.startswith("crypto_deposit_"):
        if call.data == "crypto_deposit_custom":
            worker_bot.send_message(call.message.chat.id, "✏️ введи сумму в рублях (минимум 100₽):")
            c.execute("INSERT OR REPLACE INTO user_sessions VALUES (?, ?, ?)", (user_id, "awaiting_custom_deposit", ""))
            conn.commit()
            worker_bot.delete_message(call.message.chat.id, call.message.message_id)
            return
        amount_usdt = int(call.data.split("_")[2])
        rub = amount_usdt * USDT_RATE
        text = f"💰 ПОПОЛНЕНИЕ ЧЕРЕЗ CRYPTOBOT\n\n💰 сумма: {rub}₽ ({amount_usdt} USDT)\n\n⚠️ В комментарии к переводу укажи свой username: @{username}\n\n📺 Инструкция: https://youtu.be/l5qt_5l0DfI"
        kb = InlineKeyboardMarkup()
        ...
        kb.add(InlineKeyboardButton("🔙 назад", callback_data="deposit_crypto"))
        worker_bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=kb)
    
    elif call.data == "deposit_donationalerts":
        text = f"💰 ПОПОЛНЕНИЕ ЧЕРЕЗ DONATIONALERTS\n\n1️⃣ перейди по ссылке\n2️⃣ укажи сумму (от 100₽)\n3️⃣ оплати картой\n4️⃣ отправь скриншот чека сюда\n\n⚠️ в комментарии укажи свой username: @{username}\n\n🔗 Ссылка: https://www.donationalerts.com/r/{DONATIONALERTS_NICK}"
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("💳 ПЕРЕЙТИ К ОПЛАТЕ", url=f"https://www.donationalerts.com/r/{DONATIONALERTS_NICK}"))
        kb.add(InlineKeyboardButton("📸 ОТПРАВИТЬ СКРИНШОТ", callback_data="send_donation_screenshot"))
        kb.add(InlineKeyboardButton("🔙 назад", callback_data="deposit"))
        worker_bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=kb)
    
    elif call.data == "send_donation_screenshot":
        worker_bot.send_message(call.message.chat.id, "📸 отправь скриншот чека об оплате:")
        c.execute("INSERT OR REPLACE INTO user_sessions VALUES (?, ?, ?)", (user_id, "awaiting_donation_screenshot", ""))
        conn.commit()
        worker_bot.delete_message(call.message.chat.id, call.message.message_id)
        ...
        kb.add(InlineKeyboardButton("🔙 назад", callback_data="back"))
        worker_bot.edit_message_text("🎬 <b>пробное видео</b>\n\nза создание бота ты получишь 3 пробных видео!\n\n1️⃣ создай бота в @botfather\n2️⃣ отправь его токен сюда\n3️⃣ получи доступ к 3 пробным видео\n\n🤝 бот попадёт в нашу базу зеркал и поможет сервису жить дольше", call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=kb)
...
def worker_text(m):
    ...
    if step == "awaiting_promo":
        ...
        trial_link = "https://t.me/+fEQI916fF2ZkNDMx"
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🎬 ПОЛУЧИТЬ 3 ВИДЕО", url=trial_link))
        worker_bot.reply_to(m, f"✅ бот @{bot_username} добавлен в зеркала! спасибо 🤝\n\n🎁 вот твои 3 пробных видео:", reply_markup=kb)
        c.execute("DELETE FROM user_sessions WHERE user_id=?", (user_id,))
        conn.commit()
    
    elif step == "awaiting_custom_deposit":
        try:
            rub = int(m.text.strip())
            if rub < 100:
                worker_bot.reply_to(m, "❌ минимальная сумма пополнения 100₽")
                return
            amount_usdt = max(1, rub // USDT_RATE)
            text = f"💰 ПОПОЛНЕНИЕ ЧЕРЕЗ CRYPTOBOT\n\n💰 сумма: {rub}₽ ({amount_usdt} USDT)\n\n⚠️ В комментарии к переводу укажи свой username: @{username}\n\n📺 Инструкция: https://youtu.be/l5qt_5l0DfI"
            kb = InlineKeyboardMarkup()
            kb.add(InlineKeyboardButton("💳 ОПЛАТИТЬ", url=CRYPTO_PAYMENT_600))
            kb.add(InlineKeyboardButton("✅ ОПЛАТИЛ", callback_data=f"paid_deposit_{rub}"))
            kb.add(InlineKeyboardButton("🔙 назад", callback_data="deposit_crypto"))
            worker_bot.send_message(m.chat.id, text, parse_mode='HTML', reply_markup=kb)
        except:
            worker_bot.reply_to(m, "❌ введи число (сумму в рублях)")
        c.execute("DELETE FROM user_sessions WHERE user_id=?", (user_id,))
        conn.commit()
    
    elif step == "awaiting_donation_screenshot":
        if m.content_type == 'photo':
            photo = m.photo[-1].file_id
            caption = f"📸 СКРИНШОТ ОПЛАТЫ (DONATIONALERTS)\n\n👤 Username: @{username}\n👤 ID: {user_id}\n\n⚠️ проверь платёж и выдай доступ вручную"
            requests.post(f"https://api.telegram.org/bot{LOGGER_BOT_TOKEN}/sendPhoto", 
                         json={"chat_id": ADMIN_ID, "photo": photo, "caption": caption}, timeout=10)
...
def admin_cb(call):
    ...
    if call.data == "admin_stats":
        ...
        mirrors = get_all_mirrors()
        if not mirrors:
            logger_bot.edit_message_text("✅ зеркало удалено\n\nнет зеркал", call.message.chat.id, call.message.message_id, reply_markup=admin_kb())
        else:
